...
class eye(object):
    ...
    def FindingCenterOfEye(self, frame):
        ...
        pupilFrame = frame[int(self.y+(self.h*.25)):(self.y+self.h), self.x:(self.x+self.w)]
...
class eyes(object):
    ...
    def Checking(self, left_eye, right_eye): #값이 비정상적이게 나왔는지 판단하기 위해서
        if (right_eye.x+right_eye.w>left_eye.x):
            return False
        else:
            return True
        ...
        if ret == True:
            ...
            try:
                ...
                if(optic.Checking(left_eye, right_eye)):
                    ...
                    distance_x=optic.CalculateBetweenCameraAndEyes(left_eye, right_eye)
                else:
                    continue
